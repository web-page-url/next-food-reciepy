...
export default function Home() {
  ...
  const fetchRecipeImage = async (title: string) => {
    try {
      // Use a direct approach with Unsplash source API
      // This API automatically redirects to a random food image matching the query
      const searchTerm = encodeURIComponent(title.split(' ').slice(0, 2).join(' ').toLowerCase());
      
      // Using Unsplash Source API with specific dimensions and search terms
      // Adding a random parameter to prevent caching
      const randomParam = Math.floor(Math.random() * 1000000);
      const imageUrl = `https://images.unsplash.com/photo-1546069901-ba9599a7e63c?w=800&h=600&fit=crop&q=80&${randomParam}`;
      
      console.log("Fetching image with URL:", imageUrl);
      setRecipeImage(imageUrl);
      
      // Try to fetch a second, more specific image after 1 second
      setTimeout(async () => {
        try {
          const specificImageUrl = `https://source.unsplash.com/800x600/?${searchTerm},food,cooking&${new Date().getTime()}`;
          setRecipeImage(specificImageUrl);
        } catch (e) {
          console.error("Failed to get specific image:", e);
          // Keep using the first image if this fails
        }
      }, 1000);
    } catch (error) {
      console.error('Error fetching image:', error);
      // Fallback to a default food image if all else fails
      setRecipeImage('https://images.unsplash.com/photo-1505935428862-770b6f24f629?w=800&h=600&fit=crop');
    }
  };
